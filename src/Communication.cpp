...
void Communication::communicate(Matrix<double>& matrix){
    ...
    MPI_Sendrecv(send_left.data(), send_left.size(), MPI_DOUBLE, _domain.neighbours[neighbour::LEFT], 51,
                 recv_left.data(), recv_left.size(), MPI_DOUBLE, _domain.neighbours[neighbour::RIGHT], 51, MPI_COMM_WORLD, &status);
    ...
    MPI_Sendrecv(send_right.data(), send_right.size(), MPI_DOUBLE, _domain.neighbours[neighbour::RIGHT], 151,
                    recv_right.data(), recv_right.size(), MPI_DOUBLE, _domain.neighbours[neighbour::LEFT], 151, MPI_COMM_WORLD, &status);
    ...
    MPI_Sendrecv(send_bottom.data(), send_bottom.size(), MPI_DOUBLE, _domain.neighbours[neighbour::BOTTOM], 251,
                 recv_bottom.data(), recv_bottom.size(), MPI_DOUBLE, _domain.neighbours[neighbour::TOP], 251, MPI_COMM_WORLD, &status);
    ...
    MPI_Sendrecv(send_top.data(), send_top.size(), MPI_DOUBLE, _domain.neighbours[neighbour::TOP], 351,
             recv_top.data(), recv_top.size(), MPI_DOUBLE, _domain.neighbours[neighbour::BOTTOM], 351, MPI_COMM_WORLD, &status);
